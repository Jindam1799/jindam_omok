...
const correctMessages = [ { cn: "绝妙！", kr: "기막힌 묘수로군!" }, { cn: "厉害！", kr: "대단하오!" }, { cn: "佩服！", kr: "감탄했소!" }, { cn: "完美无瑕！", kr: "완벽하오!" } ];
const wrongMessages = [ { cn: "可惜！", kr: "아깝도다!" }, { cn: "大意了！", kr: "방심했군!" }, { cn: "差之毫厘！", kr: "아주 조금 어긋났소!" }, { cn: "兵家大忌！", kr: "병법의 큰 금기로다!" } ];
...
function initAudio() {
    ...
    // 💡 iOS/iPadOS 사파리 환경에서 목소리가 즉시 로드되지 않는 문제 방지
...
function playRandomMainBGM() {
    ...
    const nextFile = mainBGMFiles[Math.floor(Math.random() * mainBGMFiles.length)];
...
function dimBGM() { if (currentMainBGM) currentMainBGM.volume = BGM_QUIZ_VOL; }
function restoreBGM() { if (currentMainBGM) currentMainBGM.volume = BGM_DEFAULT_VOL; }
...
// 💡 베트남어(광둥어)로 들리는 현상 완벽 차단!
function getChineseVoices() {
    const voices = window.speechSynthesis.getVoices();
    
    // 1. 홍콩(광둥어: zh-HK)을 철저하게 배제하고, 표준 중국어(zh-CN)와 대만(zh-TW)만 우선 추출합니다.
    let zhVoices = voices.filter(v => {
        const lang = v.lang.replace('_', '-').toLowerCase();
        return lang === 'zh-cn' || lang === 'zh-tw';
    });

    // 2. 만약 기기에 표준어가 설치 안 되어있다면, 최소한 'hk'가 안 들어간 중국어만 추출합니다.
    if (zhVoices.length === 0) {
        zhVoices = voices.filter(v => v.lang.toLowerCase().startsWith('zh') && !v.lang.toLowerCase().includes('hk'));
    }
    
    let female = null, male = null;
    
    if (zhVoices.length > 0) {
        female = zhVoices.find(v => v.name.toLowerCase().includes('female') || v.name.includes('Xiaoxiao') || v.name.includes('Tingting') || v.name.includes('Yaoyao') || v.name.includes('Mei-Jia') || v.name.includes('Lili'));
        male = zhVoices.find(v => v.name.toLowerCase().includes('male') || v.name.includes('Yunxi') || v.name.includes('Yunjian') || v.name.includes('Kangkang') || v.name.includes('Qiang') || v.name.includes('Han'));
...
function playQuestionTTS() {
    ...
    // 💡 어떤 목소리가 걸리든 엔진 자체를 강제로 표준 중국어로 고정
    utter.lang = 'zh-CN'; 
    utter.rate = 0.8; 
    utter.volume = 1.0; 
...
function replayQuestionTTS() { playQuestionTTS(); }
...
function prepareGame() {
    initAudio(); 
    const selectedLessons = document.getElementById('lesson-select').value.split(',').map(Number);
    currentLessonPool = allQuizData.filter(item => selectedLessons.includes(item.lesson));
    ...
    document.getElementById('mode-indicator').innerText = `학습 범위: [${selectedLessons[0]}과 ~ ${selectedLessons[1]}과]`;
}
...
function undoMove() {
    ...
    if (lastMove.color !== 'none') {
        ...
        const targetCell = document.querySelector(`.cell[data-row="${lastMove.r}"][data-col="${lastMove.c}"]`);
...
function restartGame() {
    showSysConfirm("정말 이 판을 다시 시작하시겠습니까?\n(현재까지 푼 문제 기록도 지워집니다)", () => {
        initBoard();
    });
}
...
function toggleTranslation() {
    ...
    if (krElem.style.display === 'none') {
        krElem.style.display = 'block'; cnElem.style.display = 'none'; 
    } else {
        krElem.style.display = 'none'; cnElem.style.display = 'block';
    }
}
...
function onCellClick(e) {
    ...
    currentQuizObj = currentLessonPool[Math.floor(Math.random() * currentLessonPool.length)];
    let wrongObj;
    do {
        wrongObj = currentLessonPool[Math.floor(Math.random() * currentLessonPool.length)];
    ...
    if (Math.random() < 0.5) {
        currentQuestionVoice = voices.female; currentAnswerVoice = voices.male;
    } else {
        currentQuestionVoice = voices.male; currentAnswerVoice = voices.female;
    }

    document.getElementById('modal-title').innerText = `[듣고 알맞은 대답 고르기]`;
    ...
    currentChoices.forEach((choice, idx) => {
        ...
        ttsBtn.className = 'choice-tts-btn'; ttsBtn.innerText = '🔊';
        ttsBtn.onclick = () => playSingleTTS(choice.text);
        
        const selectBtn = document.createElement('button');
        selectBtn.className = 'choice-btn-new'; selectBtn.innerText = choice.text;
        selectBtn.onclick = () => handleAnswer(idx);

        row.appendChild(ttsBtn); row.appendChild(selectBtn); container.appendChild(row);
    });
...
function showToast(isCorrect, cnAnswerText = "", krAnswerText = "") {
    ...
    toast.className = ''; clearTimeout(toastTimeoutAnim);

    const messages = isCorrect ? correctMessages : wrongMessages;
    const msg = messages[Math.floor(Math.random() * messages.length)];

    toastCn.innerText = msg.cn; toastKr.innerText = msg.kr;

    if (isCorrect) {
        toast.classList.add('toast-correct'); toastAns.style.display = 'none';
    } else {
        toast.classList.add('toast-wrong'); toastAns.style.display = 'block';
...
function handleAnswer(selectedIdx) {
    ...
    reviewLog.push({
        qCn: currentQuizObj.cnA, qKr: currentQuizObj.krA,
        aCn: currentQuizObj.cnB, aKr: currentQuizObj.krB,
        isCorrect: isCorrect
    });
    
    if (isCorrect) {
        placedColor = currentPlayer; playCorrectSound(); showToast(true);
    } else {
        playWrongSound(); showToast(false, currentQuizObj.cnB, currentQuizObj.krB); placedColor = 'gray'; 
    }
...
function showReviewModal() {
    ...
    if (reviewLog.length === 0) {
        ...
        reviewLog.forEach((item, idx) => {
            ...
            const mark = item.isCorrect ? '<span class="review-mark correct">[O 맞음]</span>' : '<span class="review-mark wrong">[X 틀림]</span>';
            div.innerHTML = `
